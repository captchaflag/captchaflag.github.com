---
layout: post
title: "Hack.lu 2014 - wiener"
date: 2014-10-23 20:42
comments: true
author: saboteuse
...
You might also need [this](https://wildwildweb.fluxfingers.net/static/chals/wiener_38ff175d336b9c75fbf1b77290978015.py).

## Analysis

When you connect to wildwildweb.fluxfingers.net on port 1426, you are greeted by a service that accepts commands to list users, register, and get a user's public key. If you register with 'r', the service creates a user for you and then discloses that the "locker" is on port 1427. The wording hints that the goal for this challenge is to ssh into port 1427 as the sheriff. Using the 'p' command, you can get the sheriff's public key:
...
This challenge also provided the source for the service listening on port 1426. Here is the important function:
...
This function generates the public and private keys used in RSA. The public key consists of the modulus *N* and public exponent *e*, and the private key consists of the modulus *N* and private exponent *d*. 
The trick to this challenge is to notice that the private exponent *d* generated by this function is small relative to *N*. The vulnerability is in the challenge's name: Wiener's attack on RSA.

## Solution
Now we know that the goal is to ssh into port 1427 on wildwildweb.fluxfingers.net as the sheriff. We can accomplish this by calculating his private exponent *d* using Wiener's attack and generating his private key.

First, we extracted *N* and *e* from the sheriff's public key:
...
We wrote a script to convert those values into integers and used [https://github.com/pablocelayes/rsa-wiener-attack](https://github.com/pablocelayes/rsa-wiener-attack) to recover *d*. 
Note: We had to increase the recursion limit with sys.setrecursionlimit(10000).

Here are the values we found for *N*, *e*, and *d*:
```python
...
We then used [https://gist.github.com/ddddavidee/b34c2b67757a54ce75cb](https://gist.github.com/ddddavidee/b34c2b67757a54ce75cb) to recover *p* and *q*:
```python
...
Finally, we could ssh into the server and grab the flag:
```bash
