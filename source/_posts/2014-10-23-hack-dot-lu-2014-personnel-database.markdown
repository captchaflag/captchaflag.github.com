---
layout: post
title: "Hack.lu 2014 - Personnel Database"
date: 2014-10-23 10:23
comments: true
...
Lots of criminals in this area work for one big boss, but we have been unable to
...
level 10. Do you think you can get around their protections? 
...
The `read_userfile()` function reads lines 256 bytes at a time.  However, when
writing the file you can write descriptions close to 512 bytes.  This allows you
to write long lines that will be treated as two lines by `read_userfile()`.
Since the original `access_level=0` line is written before the description, an
additional line can overwrite the `access_level` variable.
